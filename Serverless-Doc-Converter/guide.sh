...
gcloud beta run deploy pdf2docx --image gcr.io/$GOOGLE_CLOUD_PROJECT/pdf2docx-converter --platform managed --region us-central1 --no-allow-unauthenticated
...
gcloud builds submit --tag gcr.io/$GOOGLE_CLOUD_PROJECT/pdf2docx-converter
gcloud beta run deploy pdf2docx-converter --image gcr.io/$GOOGLE_CLOUD_PROJECT/pdf2docx-converter --platform managed --region us-central1 --memory=2Gi --no-allow-unauthenticated --set-env-vars DOCX_BUCKET=$GOOGLE_CLOUD_PROJECT-docx-done
